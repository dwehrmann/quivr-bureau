import json
import operator
from typing import (Annotated, AsyncIterable, List, Optional, Sequence,
                    TypedDict)
...
from modules.tools import (EmailSenderTool, ImageGeneratorTool, URLReaderTool,
                           WebSearchTool)
...
class CSWriter(KnowledgeBrainQA):
    """
    ...
    async def generate_stream(
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a professional writer in the B2B field. {custom_personality}",
                ),
    ...
    def generate_answer(
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are a professional writer in the B2B field adept at creating content for a variety of business assets. {custom_personality}",
                ),
                MessagesPlaceholder(variable_name="chat_history"),
                ("human", "{question}"),
            ]
